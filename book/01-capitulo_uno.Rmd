...
En la sección \@ref(paqueterias-r) de este capítulo, empezamos preparando la caja de herramientas que necesitamos para armar nuestra paquetería de optimización de portafolios en **R**. En particular resulta práctico conocer óomo se contruyen los objetos **xts**, éstos objetos contienen series de tiempo.
...
El cálculo del *rendimiento acumulado* nos permitirá entender los *precios de cierre ajustados* (usando la función `get_cumRet_from_dlyChg()`) y el *valor de un portafolio*, cómo se cubre en las secciones \@ref(precios-ajustados) y \@ref(valor-portafolio) En los cursos básicos de matemáticas financieras regularmente se muestran ejemplos donde se fijan los pesos iniciales de las componentes de los portafolios; sin embargo, los pesos de los portafolios cambian conforme se mueven los precios de sus componentes y es necesario volver a fijarlos a través de *rebalanceos* como se muestra en la sección \@ref(rebalanceo)
...
Notemos que adicionalemente, a lo largo de este primer capítulo se presentan los conceptos *liquidez*, *precios ajustados*, *costos de operación*, *posiciones largas y cortas* y también se mencionan algunos jugadores importantes en la industria como son los *proveedores de índices*, *los proveedores de tickers*, las *casas de bolsa* y los *custodios*. 

## Paqueterías de **R** {#paqueterias-r}

A continuación describimos todas las paqueterías de **R** que necesitamos para cargar las funciones que usaremos a lo largo de todo este trabajo.

* **xts** (https://github.com/joshuaulrich/xts): es una paquetería en **R** para manejo de objetos que contienen series de tiempo. El nombre **xts** se refiere a *extensible time series* porque esta paquetería *extiende* las propiedades de **ts** y **zoo**, paqueterías de **R** más simples para manejar series de tiempo). En este trabajo vamos a utilizar la versión 0.10-1.

A lo largo de este trabajo nos iremos familiarizando con las funciones de **xts** o **zoo**. Cuando se use por primera vez un método de una paquetería se intentará hacer explícito (e.g., `zoo::index()`, `lag.xts()`).

* **quantmod** versión 0.4-12 (https://github.com/joshuaulrich/quantmod): es una paquetería que proporciona una plantilla  que nos va a servir para manejar y visualizar de forma sencilla series de datos financieros.

* Para los problemas de optimización usaremos **nloptr** (1.0.4), esta librería envuelve una serie de rutinas de código abierto de distintos autores.

* **tidyverse**, **mltools** y **Rcpp**: estas paqueterías nos facilitan la manipulación de los objetos en **R** y nos ayudan a mejorar la eficiencia de  algoritmos.
...
```{r}
# `getSymbols(IBM)`, `getSymbols(IBM, src = "yahoo")` o `getSymbols.yahoo(IBM)` son equivalentes
...
Vamos a crear una función que llamaremos `plot_xts` que recibe un objeto `xts` y que utiliza `ggplotly` para graficar series de tiempo. Esta será la primer función de nuestra paquetería de optimización de portafolios.
...
Veamos las gráficas de los precios ajustados diarios en `herdez`y `gigante`!
...
Podemos ver que las acciones de Herdez son líquidas mientras que las acciones de Gigante no lo son.
...
>
Como vimos, `getSymbols.yahoo()` nos proporciona la serie diaria de volumen para una acción. En este trabajo no nos detendremos en cómo obtener la rotación de títulos, tampoco veremos cuánta líquidez es deseable en un intrumento. Para nuestro propósito nos basta entender que si **el precio de cierre de una acción se mantiene estática por varios días, entonces es una acción poco líquida**.
>
...
Para trabajar con otras series extraídas con `getSymbols.yahoo()` y pegadas en un mismo objeto `xts` vamos a crear la función `get_prices_yahoo()`. 
...
Ponemos `"Adjusted"` como valor predeterminado de un parámetro de `get_prices_yahoo()` que llamaremos `column`, aunque lo podremos cambiar por `"Open"`, `"High"`, `"Low"`, `"Close"` y `"Volume"`.
...
```{r}
price <- get_prices_yahoo(c('IBM', 'herdez.mx', 'gigante.mx', 'MXN=X'),
                           from = '2016-12-31',
                           to = '2017-12-31')
head(price)
```
...
```{r, precios, fig.cap='Precios de Acciones'}
plot_xts(price)
```
...
Por otro lado, un peso dejó `r sprintf("%0.2f", (pT / p0)[ , 'MXN.X.Adjusted'])` dólares. 
**Los cambios de precio en distintos tipos de cambio no son comparables**. Más adelante hablaremos a detalle sobre esto.
...
apply(dlyChg, 2, prod)[c('IBM.Adjusted', 'IBM_MXN.Adjusted', 'MXN.X.Adjusted')] - 1
```
...
* En contraste teníamos que el rendimiento en dólares de IBM fue `r sprintf("%0.2f", (prod(dlyChg[, 'IBM.Adjusted']) - 1) * 100)`%.

**¡Ojo! Un error común** es pensar que si en el periodo $\mathbb{T}$ IBM tuvo un rendimiento en dólares de $r^\text{ibm}_t$ (`r sprintf("%0.2f", (prod(dlyChg[, 'IBM.Adjusted']) - 1) * 100)`%) y el dólar tuvo un rendimiento en pesos de $r^\text{mxn/usd}_t$ (`r sprintf("%0.2f", (prod(dlyChg[, 'MXN.X.Adjusted']) - 1) * 100)`%), entonces el rendimiento en pesos de IBM es la suma de los dos $r^\text{ibm.mxn}_t = r^\text{ibm}_t + r^\text{mxn/usd}_t$ (`r sprintf("%0.2f", (prod(dlyChg[, 'IBM.Adjusted']) - 1) * 100 + (prod(dlyChg[, 'MXN.X.Adjusted']) - 1) * 100)`%). Como ya vimos, hacer esto no es correcto.

Agregaremos a nuestra paquetería la función `get_dlyChg_from_price()`. El código de la función se encuentra en el anexo __REF__. De aquí en adelante usaremos esta función para obtener series de cambios diarios a partir de precios.
...
```{r, cum-ret, fig.cap="Retornos acumulados"}
plot_xts(cumRet)
```

¡Esta gráfica es mucho más útil que la figura \@ref(fig:precios)! Los rendimientos acumulados, a diferencia de los precios, nos permite comparar facilmente cuánto cambió el valor de nuestros activos en el tiempo.
...
Para obtener rendimientos acumulados a partir de cambios diarios, agregaremos a nuestra paquetería la función `get_cumRet_from_dlyChg()`. Esta función se encuentra en el anexo __REF__.
...
```{r, close-adjusted, fig.cap="Precios de cierre y ajustados"}
...
Para la construcción de portafolios en este trabajo **vamos a suponer que podemos comprar fracciones de títulos de acciones**. Para ejemplificar, supongamos que tenemos 100 pesos y que hay una acción que cuesta 20 pesos. Si queremos inverir 25% de nuestro capital en esa acción, entonces compraríamos 1.25 títulos.
...
Supongamos que tenemos un portafolio con $n$ activos y el **peso inicial de apertura de las componentes del portafolio** esta dado por $H^0 = (H_1^0, H_2^0, \dots, H_n^0)$ con $\sum H_i^0 = 1$. Con esto, tenemos que 
...
* La **contribución del activo $i$ al rendimiento acumulado del portafolio al tiempo $t$** esta dada por
...
```{r, contrib, fig.cap='Contribuciones al rendimiento de un portafolio'}
...
plot_xts(portfolio_weights) + 
  labs(title = 'Pesos de cierre de un portafolio')
```

Veamos las diferencias entre los pesos iniciales de las componentes del portafolio y sus pesos al 30 de junio de 2017:
...
Con este cambio, ¿cuál es el rendimiento rendimiento acumulado en 2017?
...
Dado este rebalanceo y bajo estos supuestos, tenemos que para $t \leq b$ el rendimiento acumulado, las contribuciones y pesos se calculan igual que antes. En el caso de $t > b$ tenemos que:
...
* El peso de cierre $h_i^t$ del activo $i$ del portafolio esta dado por
...
El anexo __REF__ describe una lógica secuencial para obtener valores de portafolios con múltiples fechas de rebalanceo. Este procedimiento se incluye en la función `get_rebalance()` que usaremos a continuación. 
...
plot_xts(cbind(rebalance$portValue, rebalance$portContrib))  + 
  labs(title = 'Contribuciones al rendimiento despues del rebalanceo')
```
...
* Por otro lado, los pesos de cierre del portafolio antes y después del rebalanceo del 30 de junio también muestran cambios como vemos en la siguiente gráfica.

```{r}
plot_xts(rebalance$portWeight) + 
  labs(title = 'Pesos de cierre despues del rebalanceo')
```
...
* S&P GSCI Crude Oil Index - índice cuyo valor se basa en la producción de petroleo
...
```{r, market-cap, out.width = "70%", echo=FALSE, fig.align="center", fig.cap="Mapa del mundo por capitalización de mercado"}
library(knitr)
...
>
La imagen figura \@ref(fig:ibm-stats) muestra una captura de pantalla de las estadísticas de IBM en https://finance.yahoo.com/quote/ibm/key-statistics. **El número total de títulos de IBM está en amarillo**.

```{r, ibm-stats, out.width = "70%", echo=FALSE, fig.align="center", fig.cap="Estadísticas de IBM"}
library(knitr)
...
```{r}
plot_xts(rebalance$portWeight) + 
  labs(title = 'Pesos de capitalizacion de mercado de un indice')
```

De esta manera concluimos que Gigante perdió valor de mercado contra Herdez durante 2017.

## ETFs {#etfs}

```{block2, type = 'resumen'}
Se le llama **Exchange Traded Fund** (ETF) a un fondo que replica el valor de algún índice o activo que no es operado en bolsa (como el oro). Una de sus cualidades es que los ETF's son listados en bolsa y pueden ser operados como cualquier acción.
```

Recordemos que un índice es un conjunto de acciones. Si quisieramos replicar el valor de un índice necesitaríamos el capital necesario para comprar todos las activos en el índice con su peso correspondiente. Para eso, necesitaríamos mucho capital. De igual manera, si quisiéramos aumentar o disminuir nuestra posición en el índice sería muy complicado porque tendríamos que comprar o vender todos los componentes con consecuencias de costos de transacción y operativos. 
...
Existen métodologías proprietarias que se usan para clasificar acciones en distintos grupos. En particular es común que se busque clasificar las empresas de acuerdo al sector industrial al que pertenecen. Este problema no es necesariamente sencillo pues existen empresas cuyos productos y servicios se encuentran muy diversificados. Un **índice sectorial** esta compuesto por acciones que pertenecen a un sector industrial específico.
...
* IYF - Dow Jones US Financials Index - sector financiero
...
1) bonos - Los bonos determinan su valor de acuerdo a movimientos de tasa, que reflejan las expectativas de la calidad del emisor del bono y del valor del dinero en el tiempo.
