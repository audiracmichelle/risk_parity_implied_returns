...
En los anexos se incluyen todas las funciones que forman parte de la paquetería `optimist`. Esta se encuentra disponible en [https://github.com/audiracmichelle/optimist](https://github.com/audiracmichelle/optimist). Para cargarla simplemente se usa

```{}
devtools::install_github("audiracmichelle/optimist")
```

## plot_xts() {#fun-plot-xts}

Esta función grafica series de tiempo a partir de un `xts`.
...
## get_prices_yahoo() {#fun-get-prices-yahoo}

Esta función construye un `xts` que contiene precios extraidos de Yahoo! Finance. 

```{r}
#' @title get_prices_yahoo
#' @description Builds an xts containing prices downloaded from 
#' Yahoo! Finance.
#' @param yahoo_id character array containining yahoo ids.
#' @param from start date. Retrieve data no earlier than this date. 
#' (2007-01-01).
#' @param to end date. (Sys.Date())
#' @param column indicating which column should be returned: "Open", 
#' "High", "Low", "Close", "Volume", ("Adjusted")
#' @param ... additional parameters
#' @param periodicity periodicity of data to query and return. Must 
#' be one of "daily", "weekly", "monthly", ("daily")
...
## get_dlyChg_from_price() {#fun-get-dlyChg-from-price}

Esta función obtiene un `xts` que contiene cambios diarios a partir de series de precios. 
...
## get_cumRet_from_dlyChg() {#fun-get-cumRet-from-dlyChg}

Esta función obtiene un `xts` que contiene retornos acumulados a partir de series de cambios diarios. 
...
## get_rebalance() {#fun-get-rebalance}

La función `get_rebalance()` recibe un `xts` que contiene las fechas y pesos de rebalanceo de un portafolio. También recibe series de cambios diarios en el objeto `dlyChg`. A partir de esto calcula el valor de un portafolio. Esta función depende de la salida de dos funciones: `get_rebalance_t2()` y `get_rebalance_`. El rebalanceo utiliza una lógica secuencial.
...
#' @title get_rebalance
#' @description Computes daily portofolio weights, value and 
#' contributions 
#' @param dlyChg xts that contains daily changes
#' @param rebWeight xts that contains rebalance weights
...
get_rebalance <- function(
  ...
  for(c in 1:ncol(dlyChg)) {
    ...
    portContrib[[c]] <- 
      get_rebalance_(dlyChg_c, rebWeight_c)$portValue #validate
  }
...
## get_w_with_geomTruncDecay() {#fun_get_w_with_geomTruncDecay}

La función `get_w_with_geomTruncDecay()` calcula pesos con decaimiento geométrico.
...
## get_meanRet_from_dlyChg() {#fun-get-meanRet-from-dlyChg}

La función `get_meanRet_from_dlyChg()` calcula el retorno promedio. El parámetro `method` determina si la función debe regresar el promedio geométrico o aritmético. El valor `halflife=NULL` indicará si la función debe regresar el rendimiento promedio sin half-life. Cuando se incluya un half-life este tiene que ser un entero $m$ entre $0$ y $T$, es decir $0<m<T$.
...
get_meanRet_from_dlyChg <- function(
  ...
  if(is.null(halflife)) {
    ...
    w <- get_w_with_geomTruncDecay(T = T, 
                                   halflife = halflife, 
                                   interval = interval)
  }

  if(method == "arithmetic") meanRet <- 
    annualization_factor * apply(w * dlyChg, 2, 
                                 sum, 
                                 na.rm = TRUE)
  if(method == "geometric") meanRet <- 
    exp(annualization_factor * apply(w * log(dlyChg), 2, 
                                     sum, 
                                     na.rm = TRUE))
...
## get_rollChg_from_dlyChg() {#fun-get-rollChg-from-dlyChg}

La función `get_rollChg_from_dlyChg()` obtiene rendimientos móviles.  El parámetro `roll` determina el número de días de la ventana móvil. El valor `trim` indica si el `xts` resultante elimina los valores de la primer ventana o no.
...
get_rollChg_from_dlyChg <- function(
  ...
  if(is.null(halflife)) {
    ...
    w <- get_w_with_geomTruncDecay(T = roll, 
                                   halflife = halflife, 
                                   interval = interval)
  }

  if(method == "arithmetic") rollChg <- 
      rollapply(zoo(dlyChg), 
                width = roll, 
                align = 'right', 
                FUN = weighted.mean, 
                w = w, 
  ...
  if(method == "geometric") rollChg <- 
      exp(rollapply(zoo(log(dlyChg)), 
                    width = roll, 
                    align = 'right', 
                    FUN = weighted.mean, 
                    w = w, 
...
## get_sdRet_from_dlyChg() {#fun-get-sdRet-from-dlyChg}

La función `weighted.sd()` se usa para calcular desviaciones standard ponderadas. Esta función se utiliza dentro de `get_sdRet_from_dlyChg()` para obtener desviaciones standard aritméticas o geométricas a partir de cambios diarios.
...
get_sdRet_from_dlyChg <- function(
  ...
  if(method == "arithmetic") sdRet <- 
    apply(sqrt(annualization_factor) * dlyChg, 
          2, 
          weighted.sd, 
          w = w, 
          na.rm = TRUE)
  if(method == "geometric") sdRet <- 
    exp(apply(sqrt(annualization_factor) * log(dlyChg), 
              2, 
              weighted.sd, 
              w = w, 
              na.rm = TRUE))
...
## plot_riskReward_from_dlyChg() {#fun-plot-riskReward-from-dlyChg}

Esta función produce una gráfica que muestra la relación riesgo-rendimiento de activos. Los valores se calculan a partir de rendimientos móviles.
...
plot_riskReward_from_dlyChg <- function(
  ...
  ggplot(data = plot_data, 
         aes(x = Riesgo, y = Rendimiento, color = Instrumento)) +
    ...
    scale_y_continuous(
      ...
      limits = c(min(0, min(plot_data$Rendimiento)), 
                 max(plot_data$Rendimiento))
    ) +
    scale_x_continuous(
      labels = function(x) paste0(x, "%"),
      limits = c(min(0, min(plot_data$Riesgo)), 
                 max(plot_data$Riesgo))
    ) +
    theme_bw() +
...
## get_covRet_from_dlyChg() {#fun-get-covRet-from-dlyChg}

Una función que calcula matrices de covarianzas a partir de cambios diarios de activos. La matriz de covarianzas considera pesos con decaimiento geométrico.
...
## get_naiveParity_weights() {#fun-get-naiveParity-weights}

Esta función calcula pesos de paridad ingenua de riesgo a la última fecha del `xts` de entrada.
...
## get_contribParity_weights() {#fun-get-contribParity-weights}

La función `get_contribParity_weights()` calcula pesos de paridad de contribución al riesgo a la última fecha del `xts` de entrada. Para esto require una función a optimizar `fn_contribParity()` y su gradiente `gr_contribParity()`.
...
## get_principalParity_weights() {#fun-get-principalParity-weights}

La función `get_principalParity_weights()` calcula pesos de paridad de contribución al riesgo a la última fecha del `xts` de entrada. Para esto require una función a optimizar `fn_principalParity()` y su gradiente `grad_principalParity()`.
...
grad_principalParity <- function(x, P, lam, K) {
  ...
  for (i in 1:(K - 1)) {
    for (j in (i + 1):K) {
      g <- g + (lam[i] * y[i]^2 - lam[j] * y[j]^2) * 
        (lam[i] * y[i] * P[,i] - lam[j] * y[j] * P[,j])
    }
  }
  1e6 * 8.0 * g
}
...
## get_riskParity_dlyWeights() {#fun-get-riskParity-dlyWeights}

Función que obtiene series de pesos diarios de paridad de riesgo. Para esto, se proporciona el nombre de una función de pesos de paridad de riesgo: `get_naiveParity_weights()`, `get_contribParity_weights()` o `get_principalParity_weights()`.

```{r}
#' @title get_riskParity_dlyWeights
#' @description Builds an `xts` that contains daily risk parity 
#' weights
#' @param dlyChg xts that contains daily changes
#' @param fun_riskParity_weights function to compute risk parity 
#' weights
#' @param halflife number of days that accumulate 0.5 density
...
get_riskParity_dlyWeights <- function(
  ...
  riskParity_dlyWeights <- 
    xts(riskParity_dlyWeights, 
        order.by = index(dlyChg)[min_window:nrow(dlyChg)])
  
  if(!is.null(roll_parity)) {
    riskParity_dlyWeights <- 
      get_rollChg_from_dlyChg(riskParity_dlyWeights, 
                              roll = roll_parity)
...
## get_dlyImpliedRet_from_dlyWeights() {#fun-get-dlyImpliedRet-from-dlyWeights}

La función `get_dlyImpliedRet_from_dlyWeights()` obtiene rendimientos implícitos a partir de los pesos diarios de un portafolio de equilibrio.
...
## get_optim_dlyWeights_from_dlyImpliedRet() {#fun-get-optim-dlyWeights-from-dlyImpliedRet}

La función `get_optim_dlyWeights_from_dlyImpliedRet()` obtiene pesos óptimos diarios a partir de rendimientos implícitos diarios.

```{r}
#' @title get_optim_dlyWeights_from_dlyImpliedRet
#' @description gets daily weights for optimal portolio from daily 
#' implied returns
#' @param dlyImpliedRet xts that contains portfolio daily implied 
#' returns
#' @param dlyChg xts that contains daily changes
#' @param halflife number of days that accumulate 0.5 density
#' @param min_window number of days in the first window
