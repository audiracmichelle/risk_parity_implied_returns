...
Por esto, en la sección \@ref(anualizacion-estimacion) se familiariza al lector con dos métodos para obtener promedios y desviaciones de rendimientos anuales. Por un lado se presenta el método de **anualización** y por otro se muestra el método de \@ref(anualizacion-moviles) **rendimientos móviles anuales**.
...
Para esto usaremos la función `get_rollChg_from_dlyChg()`, contenida en el anexo __REF__ por brevedad.
...
Entonces la **desviación standard geométrica** ponderada de los rendimientos diarios es __BIB__
...
La dispersión aritmética la guardamos en `deviation_a`.
...
$$
\prod_{t=252}^T \left( ({\hat\mu}^{t-252,t})^{252} \right)^\frac{1}{T-252}
$$
...
A las líneas rojas en la figura __REF__ las llamaremos líneas de riesgo-rendimiento. La pendiente de una línea en la figura representa el número de unidades de rendimiento por cada unidad de riesgo que logran los portafolios que se encuentran sobre esa línea.
...
>
* el vector de rendimientos esperados $\mu = (\mu^1, \mu^2, \dots, \mu^n)$, y
* la matrix de covarianzas $\Sigma$.

```{block2, type = 'resumen'}
Usando el supuesto que los manejadores de portafolios basan sus decisiones de inversión primordialmente con $\mu$ y $\Sigma$, **Harry Markovitz** definió los principios de la **Teoría Moderna de Portafolios** a partir de la cual se construyen portafolios con base en combinaciones óptimas de retornos esperados y apetitos de riesgo __BIB__. Incluimos aquí la formulación básica de la optimización de media-varianza de Markovitz.
...
>
A partir de la formulación de este problema de optimización podemos apreciar que de acuerdo con Markovitz el objetivo del inversionista es maximizar su retorno sujeto a un nivel de riesgo objetivo. 
>
Por esto, la teoría de Markovitz regularmente se asocia con la idea de diversificación. Sin embargo, **la optimización de media-varianza no se basa en ninguna medida de diversificación**. Para entender esto, en el siguiente capítulo introducimos el concepto de fuentes de riesgo. 
...
Por ejemplo, si se invierte el 25\% de un portafolio en IYE y el resto en efectivo:
...
Con esto, hemos construido un portafolio cuyas asignaciones en riesgo de mercado y petróleo se encuentra equilibradas. De esta manera, **hemos diversificado el riesgo del portafolio proveniente de estas dos fuentes de riesgo**. Este ejemplo es una motivación para los métodos que presentaremos en el siguiente capítulo donde haremos optimización de diversificación de portafolios.
